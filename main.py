...
tree_img = pygame.image.load('asset/tree.png')
...
def place_road(grid, start_x, start_y, end_x, end_y, surface):
    if start_x == end_x:
        for y in range(min(start_y, end_y), max(start_y, end_y) + 1):
            if grid[y][start_x] == 0 and (
                start_x == 0 or grid[y][start_x - 1] == 0) and (
                start_x == GRID_SIZE - 1 or grid[y][start_x + 1] == 0):
                grid[y][start_x] = 1
                rotated_img = pygame.transform.rotate(road_straight_img, 90)
                surface.blit(rotated_img, (start_x * CELL_SIZE, y * CELL_SIZE))
    elif start_y == end_y:
        for x in range(min(start_x, end_x), max(start_x, end_x) + 1):
            if grid[start_y][x] == 0 and (
                start_y == 0 or grid[start_y - 1][x] == 0) and (
                start_y == GRID_SIZE - 1 or grid[start_y + 1][x] == 0):
                grid[start_y][x] = 1
        ...
        for x in range(min(start_x, end_x), max(start_x, end_x) + 1):
            if grid[start_y][x] == 0 and (
                ...
                surface.blit(road_straight_img, (x * CELL_SIZE, start_y * CELL_SIZE))
        for y in range(min(start_y, end_y), max(start_y, end_y) + 1):
            if grid[y][end_x] == 0 and (
                end_x == 0 or grid[y][end_x - 1] == 0) and (
                end_x == GRID_SIZE - 1 or grid[y][end_x + 1] == 0):
                grid[y][end_x] = 1
                rotated_img = pygame.transform.rotate(road_straight_img, 90)
...
def place_trees(grid, tree_count, image, surface):
    for _ in range(tree_count):
        while True:
            x = random.randint(0, GRID_SIZE - 1)
            y = random.randint(0, GRID_SIZE - 1)
            if grid[y][x] == 0:  # Ensure tree is placed on an empty cell
                surface.blit(image, (x * CELL_SIZE, y * CELL_SIZE))
                grid[y][x] = 2  # Mark the cell with a 2 to indicate a tree
                break

def generate_map():
    grid = [[0] * GRID_SIZE for _ in range(GRID_SIZE)]
    surface = pygame.Surface((MAP_SIZE, MAP_SIZE))
    surface.fill(GREEN)  # Fill with green background
    ...
    # Place trees
    place_trees(grid, 50, tree_img, surface)
